...
(function (document, $, wordfind) {
  ...
  */
  ...
  const audioEnero = document.querySelector('.audio-enero');
  const audioFebrero = document.querySelector('.audio-febrero');
  const audioMarzo = document.querySelector('.audio-marzo');
  const audioAbril = document.querySelector('.audio-abril');
  const audioMayo = document.querySelector('.audio-mayo');
  const audioJunio = document.querySelector('.audio-junio');
  const audioJulio = document.querySelector('.audio-julio');
  const audioAgosto = document.querySelector('.audio-agosto');
  const audioSeptiembre = document.querySelector('.audio-septiembre');
  const audioOctubre = document.querySelector('.audio-octubre');
  const audioNoviembre = document.querySelector('.audio-noviembre');
  const audioDiciembre = document.querySelector('.audio-diciembre');
    ...
    */
    var drawWords = function (el, words) {
      ...
      for (var i = 0, len = words.length; i < len; i++) {
        var word = words[i];
        output += '<button class="word ' + word + ' lists">' + word;
      }
      output += '</ul>';
    ...
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          ...
          if (wordList[i] == 'SEPTIEMBRE') {
            this.classList.add('a-right');
            audioSuccess.play();
            setTimeout(function () {
              audioSeptiembre.play();
            }, 200);
          }

          if (wordList[i] == 'OCTUBRE') {
            this.classList.add('a-right');
            audioSuccess.play();
            setTimeout(function () {
              audioOctubre.play();
          ...
          // $('.' + curWord).addClass('wordFoundColor');
          // var randomColor = "#"+((1<<24) * Math.random()|0).toString(16); 
          $('.' + curWord).css('background', color);

          var miCadena = $('#colorDisponibles').val();
          var divisiones = miCadena.split(",", 12);

          console.log(divisiones);

          var Total = divisiones.length;
          for (i = 0; i < Total; i++) {
            aleatorio = Math.floor(Math.random()*(divisiones.length));
            seleccion = divisiones[aleatorio];
            // alert(seleccion);
            $("#color").attr('value', seleccion);
            // divisiones.splice(aleatorio, 1);
          }

          divisiones.splice(aleatorio, 1);
          $("#colorDisponibles").attr('value', divisiones);
        }else {
